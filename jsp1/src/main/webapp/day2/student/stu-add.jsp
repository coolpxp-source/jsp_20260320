...
<body>
	<%@ include file="../../db.jsp" %>
	<form action="stu-add-result.jsp" method="get" name="stuAdd">
		<table>
			<tr>
				<th>학번</th>
...
<script>
	function fnAdd(){
		let stuAdd = document.stuAdd;
		if(stuAdd.stuNo.value.length == 0 || stuAdd.stuName.value.length == 0){
			alert("학번과 이름은 빈 값일 수 없습니다.");
			return;
		}
		if(stuAdd.stuNo.value.length != 8){
			alert("학번은 8자리여야합니다.");
			return;
		}
		stuAdd.submit();
